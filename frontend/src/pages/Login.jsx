  ...
  const loginuser = async (e) =>{
    ...
    try {
      ...
      if (data.error){
        ...
        setData({});
        toast.success('Login is successful, Welcome!')
        navigate('/dashboard')
      }
    }
    catch (error) {
  ...
  <div className = "flex rounded-2xl p-5 ">
    ...
    <div className=" sm:w-1/2 px-16 bg-white rounded-2xl border shadow-lg">
      ...
      <form onSubmit={loginuser} className="flex flex-col gap-4">
        ...
        <input className="p-2 rounded-xl border-solid" type="text" name="email" placeholder="Email" value={data.email} onChange={(e)=> setData({...data, email: e.target.value})} ></input>
        <div className="relative">
        <label htmlFor="Password">Password</label>
        <input className="mt-2 p-2  rounded-xl w-full border-solid" type="password" name="password" value={data.password} onChange={(e)=> setData({...data, password: e.target.value})} placeholder="Password" ></input>
